...
const TRACK_COLORS = { eCKM:"#3b82f6", CKM:"#8b5cf6", MSK:"#22c55e", BH:"#f97316" };
const RURAL_MEDICARE = 12;
const ADJ = { oatM1to18: 0.50, sst: 0.90, coaCap: 0.50, ssaCap: 0.25 };
const RECON_MONTHS = [6,12,18,24,30,36];
const COHORT_YEAR = { 1:"Y1", 2:"Y2", 3:"Y3" };

const DEFAULTS = {
  totalPanel: 182774,
  eligible: { eCKM:89703, CKM:107862, MSK:69416, BH:83044 },
  growthY2: 0.40, growthY3: 0.40,
  ...
  ruralPct: 0.15,
  pearlShare: 0.20,
  oar: { eCKM:0.55, CKM:0.55, MSK:0.50, BH:0.50 },
  oatM19to36: 0.625,
  ssr: { eCKM:0.92, CKM:0.92, MSK:0.90, BH:0.92 },
};

const SECTION_KEYS = {
  panel:       ["totalPanel","eligible","growthY2","growthY3"],
  enrollment:  ["penetrationMode","penetrationUniform","penetrationByTrack","rampPeriod","controlGroupByYear"],
...
// ── ENGINE ───────────────────────────────────────────────────────────────────
function runModel(inp, activeTracks) {
  const tracks = activeTracks.length > 0 ? activeTracks : TRACKS;

  // Step 1: panel growth
  const prop = {};
  TRACKS.forEach(t => { prop[t] = inp.eligible[t] / inp.totalPanel; });

  const newPanelY2 = inp.totalPanel * inp.growthY2;
  const newPanelY3 = inp.totalPanel * (1 + inp.growthY2) * inp.growthY3;
  const newEligY2 = {}, newEligY3 = {};
  TRACKS.forEach(t => { newEligY2[t] = newPanelY2 * prop[t]; newEligY3[t] = newPanelY3 * prop[t]; });
  const eligByYear = { Y1: { ...inp.eligible }, Y2: {}, Y3: {} };
  TRACKS.forEach(t => {
  ...
  cohortDefs.forEach(cd => {
    ...
    const ctrlAdj = inp.controlGroupByYear[COHORT_YEAR[cd.mc]] ? 0.90 : 1.0;
    tracks.forEach(t => {
  ...
  for (let T = 1; T <= 36; T++) {
    const byTrack = {};
    TRACKS.forEach(t => { byTrack[t] = { initial:0, followOn:0, paid:0, withheld:0, rural:0 }; });
    const byCohort = { 1:0, 2:0, 3:0 };

    subCohorts.forEach(sc => {
      if (T < sc.enrollM) return;
      const age      = T - sc.enrollM + 1;
      const cpMonth  = ((age - 1) % 12) + 1;
      const isInitial = RATES[sc.track].noFollowOn || age <= 12;
      const isPaid   = cpMonth <= 6;
      const live     = sc.inc * Math.pow(1 - monthlyChurn, age - 1);
      const rate     = RATES[sc.track].monthly[isInitial ? "initial" : "followOn"] * rateMultiplier;
      const rev      = live * rate;
    ...
    const bhI = byTrack.BH.initial, bhF = byTrack.BH.followOn, bhTot = bhI + bhF;
    const bhRate = bhTot === 0
      ? RATES.BH.monthly.initial * rateMultiplier
      : (bhI * RATES.BH.monthly.initial + bhF * (RATES.BH.monthly.followOn || 0)) / bhTot * rateMultiplier;
    const discount = tracks.length > 1 ? overlapPts * bhRate * 0.05 : 0;

    let grossRev = 0, paidRev = 0, withheldRev = 0, ruralRev = 0;
    tracks.forEach(t => {
    ...
    rawMonths.push({ T, byTrack, byCohort, grossRev, paidRev, withheldRev, ruralRev, discount, totalEnrolled });
  }

  // Step 6: adjustments
  const completionRate = 1 - inp.churnRate;
  const effOAR = {}, coa = {}, ssa = {}, applied = {}, appliedType = {};
  TRACKS.forEach(t => {
    effOAR[t] = inp.oar[t] * completionRate;
    const computeCOA = oat => effOAR[t] >= oat ? 0 : Math.min(1 - effOAR[t] / oat, ADJ.coaCap);
  ...
  const withheldAccum = {};
  TRACKS.forEach(t => { withheldAccum[t] = 0; });

  let pearlCumul = 0;
  const full = rawMonths.map(m => {
    const isRecon   = RECON_MONTHS.includes(m.T);
    const oatPeriod = m.T <= 18 ? "early" : "late";

    let netPaid = 0, release = 0;
    tracks.forEach(t => {
  ...
  const blendedAdj = (() => { let n=0,d=0; full.forEach(r=>{n+=r.blendAdj*r.grossRev;d+=r.grossRev;}); return d>0?n/d:0; })();

  // Per-track Pearl revenue by year (for chart)
  const trackRevByYear = {};
  tracks.forEach(t => { trackRevByYear[t] = { Y1:0, Y2:0, Y3:0 }; });
  full.forEach(r => {
    const totalGross = tracks.reduce((s,t) => s + (r.byTrack[t].paid + r.byTrack[t].withheld), 0);
    if (totalGross === 0) return;
    const yk = r.T<=12 ? "Y1" : r.T<=24 ? "Y2" : "Y3";
    tracks.forEach(t => {
  ...
  const kpiByYear = [0,1,2].map(i => {
    ...
    return {
      ...
      blendAdj: (() => { let n=0,d=0; slice.forEach(r=>{n+=r.blendAdj*r.grossRev;d+=r.grossRev;}); return d>0?n/d:0; })(),
      isTotal: false,
    };
  });
  ...
  return {
    ...
    prop, eligByYear, newEligY2, newEligY3, trackRevByYear, kpiByYear,
  };
}
...
const fmtYAxis = v => v>=1e6?`$${(v/1e6).toFixed(1)}M`:v>=1e3?`$${(v/1e3).toFixed(0)}K`:`$${v}`;

// ── UI COMPONENTS ─────────────────────────────────────────────────────────────
function Section({ title, children, onReset }) {
  const [open, setOpen] = useState(true);
  return (
    <div className="border border-gray-200 rounded mb-3">
      <div className="flex items-center bg-gray-50 px-3 py-2">
        <button className="flex-1 text-left font-semibold text-sm flex items-center gap-1" onClick={()=>setOpen(o=>!o)}>
          <span>{open?"▲":"▼"}</span><span>{title}</span>
        </button>
        <button onClick={onReset} className="text-xs text-blue-600 hover:text-blue-800 border border-blue-300 rounded px-2 py-0.5 ml-2">
          Reset section
        </button>
      </div>
  ...
  return (
    <div className="flex items-start gap-2">
      <div className="w-44 shrink-0 text-xs text-gray-600 pt-1">
        {label}{tooltip && <span className="ml-1 text-gray-400 cursor-help" title={tooltip}>ⓘ</span>}
      </div>
...
function NumInput({ value, onChange, min=0 }) {
  return <input type="number" value={value} min={min} onChange={e=>onChange(parseFloat(e.target.value)||0)} className="w-full border rounded px-2 py-1 text-sm" />;
}

function PctSlider({ value, onChange, min=0, max=1, step=0.005 }) {
  return (
    <div className="flex items-center gap-2">
      <input type="range" min={min} max={max} step={step} value={value}
        onChange={e=>onChange(parseFloat(e.target.value))} className="flex-1" />
      <input type="number" value={(value*100).toFixed(1)} step={step*100} min={min*100} max={max*100}
        onChange={e=>onChange((parseFloat(e.target.value)||0)/100)} className="w-16 border rounded px-2 py-1 text-sm" />
      <span className="text-xs text-gray-500">%</span>
    </div>
  );
}

function TrackFilter({ activeTracks, onChange }) {
  const pillColors = { eCKM:"bg-blue-100 text-blue-800 border-blue-300", CKM:"bg-purple-100 text-purple-800 border-purple-300", MSK:"bg-green-100 text-green-800 border-green-300", BH:"bg-orange-100 text-orange-800 border-orange-300" };
  const toggle = t => {
    if (activeTracks.includes(t)) { if (activeTracks.length===1) return; onChange(activeTracks.filter(x=>x!==t)); }
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <button onClick={()=>onChange(allOn?[TRACKS[0]]:[...TRACKS])}
        className={`text-xs px-2 py-0.5 rounded border ${allOn?"bg-gray-700 text-white border-gray-700":"bg-white text-gray-600 border-gray-300"}`}>All</button>
      {TRACKS.map(t=>(
        <button key={t} onClick={()=>toggle(t)}
          className={`text-xs px-2 py-0.5 rounded border font-medium transition-opacity ${pillColors[t]} ${activeTracks.includes(t)?"opacity-100":"opacity-30"}`}>
          {activeTracks.includes(t)?"✓ ":""}{t}
        </button>
      ))}
    </div>
  );
...
function exportCSV(months, tracks) {
  const hdr = ["Month","Year","Recon","Total Enrolled",...tracks,"C1","C2","C3","Gross Rev","Paid","Withheld","Released","Adj%","Net Rev","Pearl Rev","Pearl Cumul"];
  const rows = months.map(r=>[
    r.T, Math.ceil(r.T/12), r.isRecon?"Y":"", Math.round(r.totalEnrolled),
    ...tracks.map(t=>Math.round(r.byTrack[t].initial+r.byTrack[t].followOn)),
    Math.round(r.byCohort[1]), Math.round(r.byCohort[2]), Math.round(r.byCohort[3]),
    r.grossRev.toFixed(2), r.paidRev.toFixed(2), r.withheldRev.toFixed(2),
    r.release.toFixed(2), (r.blendAdj*100).toFixed(1)+"%",
    r.netRev.toFixed(2), r.pearlRev.toFixed(2), r.pearlCumul.toFixed(2),
  ]);
  const csv = [hdr,...rows].map(r=>r.join(",")).join("\n");
  const blob = new Blob([csv],{type:"text/csv"});
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a"); a.href=url; a.download="access_model.csv"; a.click();
}
...
export default function App() {
  ...
  const resetSection = useCallback(key => {
    setInp(prev => {
      const next = JSON.parse(JSON.stringify(prev));
      SECTION_KEYS[key].forEach(k => { next[k] = JSON.parse(JSON.stringify(DEFAULTS[k])); });
      return next;
    });
  }, []);

  const resetAll = () => { setInp(JSON.parse(JSON.stringify(DEFAULTS))); setActiveTracks([...TRACKS]); };
  ...
  const kpiTableRows = [
    ...kpiByYear,
    { label:"3-Year Total", gross:kpi.grossOAP3Y, net:kpi.netOAP3Y, pearl:kpi.pearl3Y, peakEnrolled:kpi.peakEnrolled, blendAdj:kpi.blendedAdj, isTotal:true },
  ];
    ...
    <div className="flex flex-col lg:flex-row gap-4 p-4 bg-gray-100 min-h-screen text-sm">
      ...
      <div className="w-full lg:w-96 shrink-0 overflow-y-auto">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={resetAll} className="text-xs bg-red-100 text-red-700 hover:bg-red-200 border border-red-300 rounded px-3 py-1 font-medium">↺ Reset All</button>
        </div>

        <Section title="Panel Size & Growth" onReset={()=>resetSection("panel")}>
          <Row label="Total Panel Size"><NumInput value={inp.totalPanel} onChange={v=>set("totalPanel",v)} min={1} /></Row>
          {TRACKS.map(t=>(
            <Row key={t} label={`${t} Eligible`}><NumInput value={inp.eligible[t]} onChange={v=>set(`eligible.${t}`,v)} min={0} /></Row>
          ))}
          <div className="text-xs text-gray-500 bg-gray-50 rounded p-2 space-y-0.5">
            {TRACKS.map(t=><div key={t}>{t}: <span className="font-medium">{fmtPct(prop[t])}</span> of panel</div>)}
          </div>
          <Row label="Y2 Growth Rate"><PctSlider value={inp.growthY2} onChange={v=>set("growthY2",v)} min={0} max={1} step={0.01} /></Row>
          <Row label="Y3 Growth Rate"><PctSlider value={inp.growthY3} onChange={v=>set("growthY3",v)} min={0} max={1} step={0.01} /></Row>
          <div className="text-xs text-gray-500 bg-gray-50 rounded p-2 space-y-0.5">
            <div className="font-medium">New eligible patients:</div>
            {TRACKS.map(t=><div key={t}>{t}: Y2 +{fmtInt(newEligY2[t])} / Y3 +{fmtInt(newEligY3[t])}</div>)}
          </div>
        </Section>
        ...
        <Section title="Enrollment & Penetration" onReset={()=>resetSection("enrollment")}>
          <Row label="Mode">
            <div className="flex gap-2">
              {["uniform","perTrack"].map(m=>(
                <button key={m} onClick={()=>{
                  if (m==="perTrack") { const v=inp.penetrationUniform; setInp(p=>({...p,penetrationMode:"perTrack",penetrationByTrack:{eCKM:v,CKM:v,MSK:v,BH:v}})); }
          ...
          {inp.penetrationMode==="uniform"
            ? <Row label="Penetration Rate"><PctSlider value={inp.penetrationUniform} onChange={v=>set("penetrationUniform",v)} min={0.01} max={1} step={0.01} /></Row>
            : TRACKS.map(t=><Row key={t} label={`${t} Penetration`}><PctSlider value={inp.penetrationByTrack[t]} onChange={v=>set(`penetrationByTrack.${t}`,v)} min={0.01} max={1} step={0.01} /></Row>)
          }
          <Row label="Ramp Period" tooltip="Months to reach full enrollment, per cohort">
            <div className="flex items-center gap-2">
              <input type="range" min={6} max={18} step={1} value={inp.rampPeriod}
                onChange={e=>set("rampPeriod",parseInt(e.target.value))} className="flex-1" />
            ...
            <div className="space-y-1.5">
              {["Y1","Y2","Y3"].map((yr,i)=>(
                <div key={yr} className="flex items-center gap-2">
                  ...
                  <button
                    onClick={()=>setInp(p=>({...p, controlGroupByYear:{...p.controlGroupByYear,[yr]:!p.controlGroupByYear[yr]}}))}
                    className={`px-3 py-0.5 rounded text-xs font-medium ${inp.controlGroupByYear[yr]?"bg-blue-600 text-white":"bg-gray-200 text-gray-600"}`}>
          ...
          <div className="text-xs text-gray-500 bg-gray-50 rounded p-2 space-y-0.5">
            ...
            {TRACKS.map(t=>{
              const pen = inp.penetrationMode==="uniform"?inp.penetrationUniform:inp.penetrationByTrack[t];
              return <div key={t}>{t}: {fmtInt(eligByYear.Y1[t]*pen*(inp.controlGroupByYear.Y1?0.9:1.0))}</div>;
            })}
          </div>
        </Section>

        <Section title="Patient Flow & Retention" onReset={()=>resetSection("flow")}>
          <Row label="Annual Churn Rate"><PctSlider value={inp.churnRate} onChange={v=>set("churnRate",v)} min={0} max={0.5} step={0.005} /></Row>
        ...
        <Section title="Financial & Revenue" onReset={()=>resetSection("financial")}>
          <Row label="Cost-Sharing Policy">
            <button onClick={()=>set("costSharingWaived",!inp.costSharingWaived)}
              className={`px-3 py-1 rounded text-sm font-medium ${inp.costSharingWaived?"bg-blue-600 text-white":"bg-gray-200 text-gray-700"}`}>
          ...
          <Row label="Rural Patient %"><PctSlider value={inp.ruralPct} onChange={v=>set("ruralPct",v)} min={0} max={1} step={0.01} /></Row>
          <Row label="Pearl Revenue Share"><PctSlider value={inp.pearlShare} onChange={v=>set("pearlShare",v)} min={0.05} max={0.5} step={0.005} /></Row>
        </Section>
        ...
        <Section title="Performance Assumptions" onReset={()=>resetSection("performance")}>
          ...
          {TRACKS.map(t=><Row key={t} label={`${t} OAR`}><PctSlider value={inp.oar[t]} onChange={v=>set(`oar.${t}`,v)} min={0} max={1} step={0.01} /></Row>)}
          <Row label="OAT (M19–36)" tooltip="Months 1–18 fixed at 50% per CMS rules">
            <PctSlider value={inp.oatM19to36} onChange={v=>set("oatM19to36",v)} min={0.60} max={0.65} step={0.005} />
          </Row>
          ...
          {TRACKS.map(t=><Row key={t} label={`${t} SSR`}><PctSlider value={inp.ssr[t]} onChange={v=>set(`ssr.${t}`,v)} min={0} max={1} step={0.01} /></Row>)}
        </Section>
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-4">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {kpiTableRows.map(row=>(
                  <tr key={row.label} className={`border-b border-gray-100 ${row.isTotal?"font-bold bg-blue-50 border-t-2 border-t-blue-200":"hover:bg-gray-50"}`}>
                    ...
                    <td className={`py-2 px-3 text-right tabular-nums font-semibold ${row.isTotal?"text-blue-700 text-base":"text-blue-600"}`}>{fmt$(row.pearl)}</td>
        ...
        <div className="bg-white rounded border border-gray-200 p-4">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={chartData} margin={{ top:4, right:16, left:16, bottom:4 }}>
              ...
              {activeTracks.map(t=>(
                <Bar key={t} dataKey={t} stackId="a" fill={TRACK_COLORS[t]}
        ...
        <div className="bg-white rounded border border-gray-200 p-3">
          ...
          <div className="overflow-x-auto">
            <table className="text-xs w-full">
              ...
              <tbody>
                {activeTracks.map(t=>{
                  const coaE=adj.coa[t].early, ssaV=adj.ssa[t], appE=adj.applied[t].early, isCoA=coaE>=ssaV;
                  return (
        ...
        <div className="bg-white rounded border border-gray-200 p-3">
          <div className="flex justify-between items-center mb-2">
            ...
            <button onClick={()=>exportCSV(months,activeTracks)} className="text-xs bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700">Export CSV</button>
          </div>
          {tableOpen && (
            <div className="overflow-x-auto">
              <table className="text-xs w-full whitespace-nowrap">
                <thead>
                  <tr className="bg-gray-50 text-gray-600">
                    <th className="px-1 py-1">Mo</th><th className="px-1 py-1">Yr</th><th className="px-1 py-1">Rec</th>
                    <th className="px-1 py-1">Enrolled</th>
                    {activeTracks.map(t=><th key={t} className="px-1 py-1">{t}</th>)}
                    <th className="px-1 py-1">C1</th><th className="px-1 py-1">C2</th><th className="px-1 py-1">C3</th>
                    <th className="px-1 py-1">Gross Rev</th><th className="px-1 py-1">Paid</th>
                    <th className="px-1 py-1">Withheld</th><th className="px-1 py-1">Released</th>
                    <th className="px-1 py-1">Adj%</th><th className="px-1 py-1">Net Rev</th>
                    <th className="px-1 py-1 text-blue-700">Pearl Rev</th><th className="px-1 py-1">Pearl Cumul</th>
                  </tr>
                </thead>
                <tbody>
                  {months.map(r=>(
                    <tr key={r.T} className={`border-t ${r.isRecon?"bg-yellow-50":""}`}>
                      ...
                      {activeTracks.map(t=><td key={t} className="px-1 py-0.5 text-right">{fmtInt(r.byTrack[t].initial+r.byTrack[t].followOn)}</td>)}
